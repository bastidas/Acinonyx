...
def create_circle_nonuniform_density(
    n_points: int, radius: float, center: tuple[float, float] = (0, 0),
    cluster_size: int = 6, cluster_spacing: float = 0.1,
...
def create_bitten_circle_points(
    n_points: int, radius: float, center: tuple[float, float] = (0, 0),
    bite_size: float = 0.3,
) -> list[tuple[float, float]]:
    """
...
def plot_trajectory_comparison(
    ...
    for i in range(len(target)):
        ...
        ax.annotate(
            ...
            bbox=dict(
                boxstyle='round,pad=0.4', facecolor='white', alpha=0.9,
    ...
    if distance_table_data:
        ...
        ax_distance_table.text(
            0.5, 0.5, 'No distance metrics data',
