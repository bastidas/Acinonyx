/**
 * Translation invariance for rigid group moves (hypergraph / LinkageDocument).
 *
 * BuilderTab.translateGroupRigid uses moveNodesFromOriginal; these tests assert that
 * a whole-mechanism translation does not mutate kinematic constraints or UI meta.
 */

import { describe, it, expect } from 'vitest'
import type { LinkageDocument, Edge, Node } from '../../../types'
import { moveNodesFromOriginal } from './nodeOperations'
...
/** Minimal open chain: three nodes, two edges (translation invariant as a rigid body). */
...
describe('moveNodesFromOriginal (translation invariance)', () => {
  it('translates all nodes by delta, preserves edge distances, roles, angles, and meta', () => {
    ...
    const { doc: after } = moveNodesFromOriginal(before, originalPositions, [dx, dy])

    for (const id of Object.keys(originalPositions)) {
      const p0 = originalPositions[id]!
      const p1 = after.linkage.nodes[id]!.position
      expect(p1[0]).toBeCloseTo(p0[0] + dx, 10)
      expect(p1[1]).toBeCloseTo(p0[1] + dy, 10)
    }
  ...
  it('preserves pairwise geometry for every node pair (rigid translation)', () => {
    const before = makeOpenChainDoc()
    ...
    const { doc: after } = moveNodesFromOriginal(before, originalPositions, [-3, 11])
