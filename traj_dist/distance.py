"""
traj_dist.distance - Main API for computing distances between 2D trajectories.

This module is based on traj-dist by Brendan Guillouet:
...
from .pydist.lcss import s_lcss
...
def sowd_grid(traj_1, traj_2, type_d='euclidean', converted=None, precision=None):
    ...
    if type_d == 'euclidean' and not converted:
        raise Warning(
            'Euclidean implementation for distance sowd_grid is not '
            'disponible if your data is not already converted in cell format',
        )

    if converted is None:
        warnings.warn(
            'converted parameter should be specified for metric sowd_grid. Default '
            'is True',
        )
        converted = True
    if converted:
        ...
        if precision is None:
            warnings.warn(
                'precision parameter should be specified for metric sowd_grid if converted '
...
def pdist(
    traj_list, metric='sspd', type_d='euclidean', converted=None, precision=None,
    ...
    if not (
        metric in [
            'sspd', 'dtw', 'lcss', 'hausdorff', 'frechet', 'discret_frechet', 'sowd_grid',
            'erp', 'edr',
        ]
    ):
        raise ValueError(
            "The metric argument should be 'sspd', 'dtw', 'lcss','erp','edr' 'hausdorff', 'frechet',"
    ...
    if type_d == 'spherical' and (metric in ['frechet', 'discret_frechet']):
        raise ValueError(
            'spherical implementation for distance ' + metric + ' is not '
            'disponible',
        )
    if type_d == 'euclidean' and (metric in ['sowd', 'sowd_grid']):
        if not (converted):
            raise ValueError(
                'Euclidean implementation for distance ' +
...
def cdist(
    traj_list_1, traj_list_2, metric='sspd', type_d='euclidean', converted=None,
