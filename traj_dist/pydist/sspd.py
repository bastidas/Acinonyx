...
from .basic_euclidean import eucl_dist_traj
from .basic_euclidean import point_to_trajectory
from .basic_spherical import great_circle_distance
from .basic_spherical import great_circle_distance_traj
from .basic_spherical import point_to_path
...
def s_spd(lons0, lats0, lons1, lats1, n0, n1, mdist, t0_dist):
    ...
    for j in range(n1):
        ...
        for i in range(n0 - 1):
            dist_j0 = np.min((
                dist_j0, point_to_path(
                    lons0[i], lats0[i], lons0[i + 1], lats0[i + 1], lons1[j],
...
def s_sspd(t0, t1):
    ...
    dist = s_spd(lons0, lats0, lons1, lats1, n0, n1, mdist, t0_dist) + s_spd(
        lons1, lats1, lons0, lats0, n1, n0,
        mdist.T, t1_dist,
    )
    return dist
